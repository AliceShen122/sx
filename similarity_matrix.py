...
def similarity_matrix():
    labels = kk.label_set  # (21072,24) -> ndarray
    dim = labels.shape[1]  # 38

    query_label = np.zeros((len(kk.indexTest), dim))  # (1984, 38)
    retrieval_label = np.zeros((len(kk.indexDatabase), dim))  # (19088, 38)

    for index, i in enumerate(kk.indexTest):
        query_label[index] = labels[i]

    for index, i in enumerate(kk.indexDatabase):
